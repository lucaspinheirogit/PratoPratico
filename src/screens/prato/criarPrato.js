...
import { View, Picker, Image, StyleSheet, FlatList, Text, ActivityIndicator } from 'react-native'
import ImagePicker from 'react-native-image-picker'
import FastImage from 'react-native-fast-image'

import { FormGroup, Input, InputNumber, TextArea, Select } from '../../styled-components/Input'
...
const Ingrediente = props => {
  ...
  return (
    <View style={styles.ingrediente}>
      ...
      <Text style={styles.remover} onPress={() => props.removerIngrediente(item.nome)}>
        Remover
      </Text>
    </View>
  )
...
class CriarPrato extends Component {
  ...
  cadastrar(nome, desc, ingredientes, modo, tempoh, tempom, tempos, dif, foto) {
    this.setState({ erro: '' })
              ...
              <FlatList
                ...
                renderItem={({ item, index }) => (
                  <Ingrediente
                    key={index}
                    item={item}
                    removerIngrediente={() => this.removerIngrediente(item.nome)}
                  />
                )}
              />
...
const mapDispatchToProps = dispatch => ({
  create(nome, desc, ingredientes, modo, tempo, dif, foto) {
    dispatch(actions.create(nome, desc, ingredientes, modo, tempo, dif, foto))
  },
})
